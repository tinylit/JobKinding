(function(window) {
    ...
    jobKinding.fn.init = function(selector, context) {
        ...
        if (typeof selector === "string") {
            ...
            if (match && (match[1] || !context)) {
                if (match[1]) {
                    context = context && context instanceof jobKinding ? context[0] : document;
                    jobKinding.merge(this, jobKinding.parseHTML(match[1], context && context.nodeType ? context.ownerDocument || context : document, true));
                    if (rsingleTag.test(match[1]) && jobKinding.isPlainObject(context)) {
                        for (match in context) {
                            if (jobKinding.isFunction(this[match])) {
        ...
        return jobKinding.makeArray(selector, this);
    }
    ...
    jobKinding.extension = function(args, callback) {
        ...
        var setConfigs = function(key) {
            ...
            if (!(value === configs || value === undefined)) {
                if (value && isTraverse && (jobKinding.isPlainObject(value) || (isArray = jobKinding.isArray(value)))) {
                    target[key] = jobKinding.extension([isTraverse, configs || (isArray ? [] : {}), value], callback);
                    isArray = false;
                } else {
    ...
    jobKinding.extend({
        ...
        makeArray: function(arr, results) {
            ...
            if (arr != null) {
                if (isArraylike(arr)) {
                    jobKinding.merge(results, typeof arr === "string" ? [arr] : arr);
                } else {
    ...
    function curCSS(elem, name, computed) {
        ...
        if (computed) {
            ...
            if (val === "" && !jobKinding.contains(elem.ownerDocument, elem)) {
                val = jobKinding.style(elem, name);
            }
            ...
            preFilter: {
                ...
                "CHILD": function(match) {
                    ...
                    if (match[1].slice(0, 3) === "nth") {
                        if (!match[3]) {
                            jobKinding.syntaxError(match[0]);
                        }
    ...
    jobKinding.extend({
        prop: function(elem, name, value) {
            ...
            var val, hooks;
            if (nodeType !== 1) {
                name = jobKinding.propFix[name] || name;
                hooks = jobKinding.propHooks[name];
            }
        ...
        attr: function(elem, name, value) {
            ...
            if (elem.getAttribute == null) {
                return jobKinding.prop.apply(this, arguments);
            }
            var val, hooks;
            if (nodeType !== 1) {
                hooks = jobKinding.attrHooks[name.toLowerCase()] || rbooleans.test(name) && boolHook;
            }
            if (arguments.length > 2) {
                if (value == null) {
                    return jobKinding.removeAttr(elem, name);
                }
                if (hooks && "set" in hooks && (val = hooks.set(elem, value, name)) != null) {
                    return val;
                }
    ...
    jobKinding.text = function(elem) {
        ...
        if (nodeType == null) {
            while (node = elem[i++]) {
                returnVal += jobKinding.text(node);
            }
            return returnVal;
        }
        if (nodeType === 1 || nodeType === 9 || nodeType === 11) {
            if (typeof elem.textContent === "string") {
                ...
                for (elem = elem.firstChild; elem; elem = elem.nextSibling) {
                    returnVal += jobKinding.text(elem);
                }
            }
            return returnVal;
        }
    ...
    jobKinding.valHooks = {
        ...
        select: {
            get: function(elem) {
                ...
                for (; i < max; i++) {
                    option = options[i];
                    if ((option.selected || i === index) && !option.disabled && (!option.parentNode.disabled || !jobKinding.nodeName(option.parentNode, "optgroup"))) {
                        value = jobKinding(option).val();
                        if (one) {
                            return value;
                        }
                        values.push(value);
            ...
            set: function(elem, value) {
                ...
                while (i--) {
                    option = options[i];
                    if (option.selected = jobKinding.indexOfArray(jobKinding.valHooks.option.get(option), values) > -1) {
                        optionSet = true;
                    }
                }
                if (!optionSet) {
        ...
        style: function(elem, name, value, extra) {
            ...
            if (value !== undefined) {
                ...
                if (type === "string" && (val = rcssNum.exec(value)) && val[1]) {
                    val = (val[1] + 1) * val[2] + jobKinding.css(elem, name, true);
                    type = "number";
                }
                ...
                if (type === "number") {
                    value += val && val[3] || (jobKinding.cssNumber[origName] ? "" : "px");
                }
    ...
    function defaultPrefilter(elem) {
        ...
        var display = jobKinding.css(tmp, "display");
    ...
    function manipulationTarget(elem, content) {
        if (jobKinding.nodeName(elem, "table") && jobKinding.nodeName(content.nodeType !== 11 ? content : content.firstChild, "tr")) {
    ...
    jobKinding.fn.extend({
        domManip: function(args, callback) {
            ...
            if (len > 0) {
                fragment = jobKinding.buildFragment(args, this[0].ownerDocument, false, this);
                first = fragment.firstChild;
                if (fragment.childNodes.length === 1) {
                    fragment = first;
                }
                scripts = jobKinding.each(getAll(fragment, "script"));
                length = scripts.length;
                if (first) {
                    for (; i < len; i++) {
                        node = fragment;
                        if (i < xlen) {
                            node = jobKinding.clone(node, true, true);
                            if (length) {
                                jobKinding.merge(scripts, getAll(node, "script"));
                            }
                        }
                ...
                if (length) {
                    ...
                    jobKinding.each(scripts, restoreScript);
                    for (i = 0; i < length; i++) {
                        node = scripts[i];
                        if (rscriptType.test(node.type) && jobKinding.contains(doc, node)) {
                            if (node.src) {
                                if (jobKinding._evalUrl) {
                                    jobKinding._evalUrl(node.src);
                                }
                            } else {
    ...
    jobKinding.extend({
        ...
        parseXML: function(data) {
            ...
            if (!xml || !xml.documentElement || xml.getElementsByTagName("parsererror").length) {
                jobKinding.error("Invalid XML: " + data);
            }
            return xml;
        },
        parseHTML: function(data, context, keepScripts) {
            ...
            parsed = jobKinding.buildFragment([data], context, scripts);
            if (scripts) {
                jobKinding(scripts).remove();
            }
            return jobKinding.merge([], parsed.childNodes);
        }
    });
        ...
        if (!arr) {
            for (arr = [], elems = context.childNodes || context;
                ...
                if (!tag || tag === "*" || jobKinding.nodeName(elem, tag)) {
                    ...
                    jobKinding.merge(arr, getAll(elem, tag));
                }
            }
        }
        return !tag || tag === "*" || jobKinding.nodeName(context, tag) ? jobKinding.merge([context], arr) : arr;
    }
    ...
    jobKinding.buildFragment = function(elems, context, scripts, selection) {
        ...
        for (; i < l; i++) {
            elem = elems[i];
            if (elem || elem === 0) {
                if (jobKinding.type(elem) === "object") {
                    jobKinding.merge(nodes, elem.nodeType ? [elem] : elem);
                    ...
                    jobKinding.merge(nodes, tmp.childNodes);
                    tmp.textContent = "";
        ...
        i = 0;
        while ((elem = nodes[i++])) {
            if (selection && jobKinding.indexOfArray(elem, selection) > -1) {
                continue;
            }
            contains = jobKinding.contains(elem.ownerDocument, elem);
    ...
    jobKinding.fn.extend({
        find: function(selector) {
            ...
            for (i = 0; i < len; i++) {
                jobKinding.expr(selector, self[i], ret);
            }
            ...
            if (cache) {
                ...
                if (cache && cache.handle && cache.events) {
                    for (var type in cache.events) {
                        jobKinding.remove(elem, type, cache.handle);
                        jobKinding.del(cache.events, type);
                    }
                    jobKinding.del(cache.events, "handle", "events");
                }
                cache = null;
            }
    ...
    jobKinding.event = {
        ...
        process: function(event) {
            ...
            handlerQueue = jobKinding.event.handlers.call(this, event, handlers);
            i = 0;
            while ((matched = handlerQueue[i++]) && !event.isPropagationStopped()) {
                ...
                while ((handleObj = matched.handlers[j++]) && !event.isImmediatePropagationStopped()) {
                    if (!event.namespace_reg || event.namespace_reg.test(handleObj.namespace)) {
                        ...
                        ret = ((jobKinding.event.fix[handleObj._type] || {}).handle || handleObj.handler).apply(matched.elem, args);
    ...
    function cloneCopyEvent(src, dest) {
        ...
        if ((cache = dataPriv.has(src)) && cache.handle && (events = cache.events)) {
            ...
            handle.elem = dest;
            for (type in events) {
                jobKinding.event.add(dest, type, handle);
            }
            dataPriv.set(dest, {
                handle: handle,
                events: jobKinding.extend({}, events, true)
            });
        }
        dest = null;
    }
    ...
    jobKinding.Event = function(src, props) {
        ...
        if (props) {
            jobKinding.extend(this, props);
        }
        this.timeStamp = src && src.timeStamp || +(new Date());

        this[jobKinding.expando] = true;
    };
    jobKinding.Event.prototype = {
    ...
    jobKinding.extend({
        ...
        on: function(elem, types, handler, data, selector) {
            ...
            if (!handler.dataGuid) {
                handler.dataGuid = jobKinding.dataGuid++;
            }
            ...
            while (i--) {
                ...
                fix = jobKinding.event[type] || {};
                type = (selector ? fix.delegateType : fix.bindType) || type;
                fix = jobKinding.event[type] || {};
                handleObj = jobKinding.extend({
                    data: data,
                    type: type,
                    _type: _type,
                    handler: handler,
                ...
                if (!(handlers = events[type])) {
                    ...
                    if (!fix.setup || fix.setup.call(elem, data, namespaces, eventHandle) === false) {
                        jobKinding.event.add(elem, type, eventHandle);
                    }
                }
                if (fix.add) {
                ...
                jobKinding.event.global[type] = true;
            }
            elem = null;
        },
        off: function(elem, types, handler, selector, mappedTypes) {
            ...
            while (i--) {
                ...
                if (!type) {
                    for (type in events) {
                        jobKinding.off(elem, type + types[i], handler, selector, mappedTypes);
                    }
                    continue;
                }
                namespaces = (tmp[2] || "").split(".").sort();
                fix = jobKinding.event.fix[type] || {};
                type = (selector ? fix.delegateType : fix.bindType) || type;
                handlers = events[type] || [];
                ...
                if (count && !handlers.length) {
                    if (!fix.teardown || fix.teardown.call(elem, namespaces, cache.handle)) {
                        jobKinding.event.remove(elem, type, cache.handle);
                    }
                    jobKinding.del(events, type);
                }
            }
            if (jobKinding.isEmptyObject(events)) {
                dataPriv.set(elem);
            }
        },
        fix: function(event) {
            ...
            event = new jobKinding.Event(originalEvent);
        ...
        trigger: function(event, data, elem, onlyHandlers) {
            ...
            event = event[jobKinding.expando] ? event : new jobKinding.Event(type, typeof event === "object" && event);
            ...
            data = data == null ? [event] : jobKinding.makeArray(data, [event]);
            fix = jobKinding.event.fix[type] || {};
            ...
            if (!onlyHandlers && !fix.noBubble && !jobKinding.isWindow(elem)) {
            ...
            if (!onlyHandlers) {
                if ((!fix._default || fix._default.apply(elem.ownerDocument, data) === false) && !(type === "click" && jobKinding.nodeName(elem, "a")) && dataPriv.receive(elem)) {
                    if (ontype && elem[type] && !jobKinding.isWindow(elem)) {
                        tmp = elem[ontype];
                        if (tmp) {
                            elem[ontype] = null;
                        }
    ...
    function buildParams(prefix, obj, traditional, add) {
        ...
        } else if (!traditional && jobKinding.type(obj) === "object") {
            for (name in obj) {
    ...
    function ajaxExtend(target, src) {
        var key, deep, flatOptions = jobKinding.ajaxSettings.flatOptions || {};
        for (key in src) {
        ...
        if (deep) {
            jobKinding.extend(true, target, deep);
        }
        return target;
    }
    ...
    jobKinding.extend({
        ...
        ajaxSettings: {
            ...
            converters: {
                "* text": String,
                "text html": true,
                "text json": jobKinding.parseJSON,
                "text xml": jobKinding.parseXML
            },
            flatOptions: {
                url: true,
                context: true
        ...
        ajax: function(url, options) {
            ...
            if (s.data && s.processData && typeof s.data !== "string") {
                s.data = jobKinding.param(s.data, s.traditional);
            }
            ...
            if (fireGlobals && jobKinding.active++ === 0) {
            ...
            if (s.ifModified) {
                if (jobKinding.lastModified[cacheURL]) {
                    jqXHR.setRequestHeader("If-Modified-Since", jobKinding.lastModified[cacheURL]);
                }
                if (jobKinding.etag[cacheURL]) {
                    jqXHR.setRequestHeader("If-None-Match", jobKinding.etag[cacheURL]);
                }
            }
            ...
            function done(status, nativeStatusText, responses, headers) {
                ...
                if (isSuccess) {
                    if (s.ifModified) {
                        ...
                        if (modified) {
                            jobKinding.lastModified[cacheURL] = modified;
                        }
                        modified = jqXHR.getResponseHeader("etag");
                        if (modified) {
                            jobKinding.etag[cacheURL] = modified;
                        }
                    }
    ...
    jobKinding.each(["get", "post"], function(method) {
        jobKinding[method] = function(url, data, callback, type) {
            ...
            return jobKinding.ajax(jobKinding.extend({
                url: url,
                type: method,
                dataType: type || "json",
                data: data,
    ...
    jobKinding.ajaxPrefilter("json jsonp", function(s, originalSettings, jqXHR) {
        ...
        if (jsonProp || s.dataTypes[0] === "jsonp") {
            ...
            jqXHR.always(function() {
                ...
                if (responseContainer && jobKinding.isFunction(overwritten)) {
    ...
    function adoptValue(value, resolve, unsolve) {
        var method;
        try {
            if (value && jobKinding.isFunction((method = value.promise))) {
                method.call(value).done(resolve).fail(unsolve)
            } else if (value && jobKinding.isFunction((method = value.then))) {
    ...
    jobKinding.extend({
        ...
        ready: function(wait) {
            ...
            jobKinding.isReady = true;
            if (wait !== true && --jobKinding.readyWait > 0) {
                return false;
            }
            readyList.resolveWith(document, [jobKinding]);
            return true;
        }
    });
    jobKinding.ready.then = readyList.then;
    ...
    if (document.readyState === "complete" || (document.readyState !== "loading" && !document.documentElement.doScroll)) {
        window.setTimeout(jobKinding.ready);
    } else {
        if (document.addEventListener) {
    ...
    window.jobKinding = window.$ = jobKinding;
