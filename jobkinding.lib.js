(function($) {
  ...
  $.lib.html = function(selector) {
    ...
    var key, rel, match, matched = true,
    ...
    matched = rsingleTag.test(htmlObj["TAG"]);
    for (key in htmlObj) {
      if (multiObj[key]) {
      ...
      switch (key) {
        ...
        case "CONTENT":
          if (matched) {
            htmls.push(($.lib.singleTag[htmlObj["TAG"].toLowerCase()] || $.lib.singleTag._default)(htmlObj["CONTENT"], htmlObj["TAG"], htmlObj));
          }
          break;
      }
